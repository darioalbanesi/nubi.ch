	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				'installing'                      => esc_html__( 'Installing Plugin: %s', 'wp-trust' ),
				...
				'dashboard'                       => esc_html__( 'Return to the dashboard', 'wp-trust' ),
				'plugin_activated'                => esc_html__( 'Plugin activated successfully.', 'wp-trust' ),
				'activated_successfully'          => esc_html__( 'The following plugin was activated successfully:', 'wp-trust' ),
				'plugin_already_active'           => esc_html__( 'No action taken. Plugin %1$s was already active.', 'wp-trust' ),
				'plugin_needs_higher_version'     => esc_html__( 'Plugin not activated. A higher version of %s is needed for this theme. Please update the plugin.', 'wp-trust' ),
				'complete'                        => esc_html__( 'All plugins installed and activated successfully. %1$s', 'wp-trust' ),
				'dismiss'                         => esc_html__( 'Dismiss this notice', 'wp-trust' ),
				'contact_admin'                   => esc_html__( 'Please contact the administrator of this site for help.', 'wp-trust' ),
			);
		...
		public function filter_plugin_action_links_update( $actions ) {
			$actions['update'] = sprintf(
				...
				esc_attr__( 'This plugin needs to be updated to be compatible with your theme.', 'wp-trust' ),
				esc_html__( 'Update Required', 'wp-trust' )
			);
		...
		protected function add_admin_menu( array $args ) {
			if ( has_filter( 'tgmpa_admin_menu_use_add_theme_page' ) ) {
				_deprecated_function( 'The "tgmpa_admin_menu_use_add_theme_page" filter', '2.5.0', esc_html__( 'Set the parent_slug config variable instead.', 'wp-trust' ) );
			}
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
				...
				if ( $this->is_tgmpa_complete() ) {
					echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'wp-trust' ) . '</a>' ), '</p>';
			...
			if ( ! empty( $desired_slug ) ) {
				...
				if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
					...
					if ( true === $GLOBALS['wp_filesystem']->move( $from, $to ) ) {
						...
						return new WP_Error( 'rename_failed', esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'wp-trust' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'wp-trust' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
					}
				} elseif ( empty( $subdir_name ) ) {
					return new WP_Error( 'packaged_wrong', esc_html__( 'The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'wp-trust' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'wp-trust' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
				}
			}
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				foreach ( $message as $type => $plugin_group ) {
					...
					$imploded       = empty( $linked_plugins ) ? $last_plugin : ( implode( ', ', $linked_plugins ) . ' ' . esc_html_x( 'and', 'plugin A *and* plugin B', 'wp-trust' ) . ' ' . $last_plugin );
