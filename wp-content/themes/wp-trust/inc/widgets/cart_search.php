...
class WC_Widget_Cart_Search extends WP_Widget {
    public function __construct() {
        parent::__construct(
            ...
            esc_html__('Cart & Search', 'wp-trust'), // Name
            array('description' => esc_html__("Display the user's Cart and Search form in the sidebar.", 'wp-trust'),) // Args
        );
    ...
    function widget($args, $instance) {
        ...
        <div class="widget_cart_search_wrap">
            ...
            <div class="shopping_cart_dropdown">
                <div class="shopping_cart_dropdown_inner">
                    ...
                    <ul class="<?php echo implode(' ', $list_class); ?>">
                        ...
                        <?php else : ?>
                            ...
                            <li class="cart-list clearfix"><?php esc_html_e( 'No products in the cart.', 'wp-trust' ); ?></li>
                ...
                <a href="<?php echo ''.$woocommerce->cart->get_cart_url(); ?>" class="btn btn-primary left wc-forward"><?php esc_html_e( 'Cart', 'wp-trust' ); ?></a>
                <span class="total right"><?php esc_html_e( 'Total', 'wp-trust' ); ?>:<span><?php echo ''.$woocommerce->cart->get_cart_subtotal(); ?></span></span>
            ...
            <div class="widget_searchform_content">
                <form role="search" method="get" action="<?php echo esc_url( home_url( '/'  ) );?>">
                    ...
                    <input type="submit" value="<?php echo esc_attr__( 'Search', 'wp-trust' )?>" />
					<?php if($woocommerce):?>
    ...
    function form( $instance ) {
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e( 'Title:', 'wp-trust' ); ?></label>
            <input id="<?php echo esc_attr( $this->get_field_id('title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('title') ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
        </p>
		<p>
            <label for="<?php echo esc_url($this->get_field_id('show_cart')); ?>"><?php esc_html_e( 'Show Cart:', 'wp-trust' ); ?></label>
			<select id="<?php echo ''.$this->get_field_id('show_cart'); ?>" class="widefat" name="<?php echo ''.$this->get_field_name('show_cart'); ?>">
				<option value="0" <?php selected($show_cart,0); ?>><?php echo esc_html__('No','wp-trust'); ?></option>
				<option value="1" <?php selected($show_cart,1); ?>><?php echo esc_html__('Yes','wp-trust'); ?></option>
			</select>
        </p>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('show_search')); ?>"><?php esc_html_e( 'Show Search:', 'wp-trust' ); ?></label>
			<select id="<?php echo ''.$this->get_field_id('show_search'); ?>" class="widefat" name="<?php echo ''.$this->get_field_name('show_search'); ?>">
				<option value="0" <?php selected($show_search,0); ?>><?php echo esc_html__('No','wp-trust'); ?></option>
				<option value="1" <?php selected($show_search,1); ?>><?php echo esc_html__('Yes','wp-trust'); ?></option>
			</select>
        </p>
    <?php
