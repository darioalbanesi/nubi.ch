...
class CS_Recent_Post_Carousel extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
                'cs_recent_post_carousel', esc_html__('CMS Recent Post','wp-trust'), array('description' => esc_html__('Recent Posts Widget.', 'wp-trust'),)
        );
    }
    ...
    function widget($args, $instance) {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Recent Posts', 'wp-trust' ) : $instance['title'], $instance, $this->id_base);
                                ...
                                <div class="cms-recent-media">  
                                   <div class="image">
                                       <a class="post-featured-img" href="<?php the_permalink(); ?>">
                                            <?php 
                                                if(has_post_thumbnail() && !post_password_required() && !is_attachment() &&  wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), '80X80', false)):
                                                    $class = ' has-thumbnail';
                                                    $thumbnail = get_the_post_thumbnail(get_the_ID(),'80X80');
                                                else:
                                                    $class = ' no-image';
    ...
    function form($instance) {
        ...
        <p>
            <label for="<?php echo ''.$this->get_field_id('number'); ?>"><?php esc_html_e( 'Number of post to show:', 'wp-trust' ); ?></label>
