...
class WP_Trust_Base
{
    ...
    public static function wp_trust_get_page_title(){
        ...
        if (!is_archive()){
            ...
            if(is_page()) :
                /* custom title. */
                if(!empty($wp_trust_meta->_cms_page_title_text) && $wp_trust_meta->_cms_page_title_text):
                    echo esc_attr($wp_trust_meta->_cms_page_title_text);
                else :
                    the_title();
                endif;
            ...
            /* author. */
            elseif ( is_author() ) :
                printf( esc_html__( 'Author: %s', 'wp-trust' ), '<span class="vcard">' . get_the_author() . '</span>' );
            /* date */
            elseif ( is_day() ) :
                printf( esc_html__( 'Day: %s', 'wp-trust' ), '<span>' . get_the_date() . '</span>' );
            elseif ( is_month() ) :
                printf( esc_html__( 'Month: %s', 'wp-trust' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'wp-trust' ) ) . '</span>' );
            elseif ( is_year() ) :
                printf( esc_html__( 'Year: %s', 'wp-trust' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'wp-trust' ) ) . '</span>' );
            /* post format */
            elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
                esc_html_e( 'Asides', 'wp-trust' );
            elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
                esc_html_e( 'Galleries', 'wp-trust');
            elseif ( is_tax( 'post_format', 'post-format-image' ) ) :
                esc_html_e( 'Images', 'wp-trust');
            elseif ( is_tax( 'post_format', 'post-format-video' ) ) :
                esc_html_e( 'Videos', 'wp-trust' );
            elseif ( is_tax( 'post_format', 'post-format-quote' ) ) :
                esc_html_e( 'Quotes', 'wp-trust' );
            elseif ( is_tax( 'post_format', 'post-format-link' ) ) :
                esc_html_e( 'Links', 'wp-trust' );
            elseif ( is_tax( 'post_format', 'post-format-status' ) ) :
                esc_html_e( 'Statuses', 'wp-trust' );
            elseif ( is_tax( 'post_format', 'post-format-audio' ) ) :
                esc_html_e( 'Audios', 'wp-trust' );
            elseif ( is_tax( 'post_format', 'post-format-chat' ) ) :
                esc_html_e( 'Chats', 'wp-trust' );
            /* woocommerce */
            elseif (class_exists('wp-trust') && is_woocommerce()):
    ...
    /* RevSlider */
    public function wp_trust_build_shortcode_rev_slider()
    {
        if (class_exists('RevSlider')) {
            ...
            if ($arrSliders) {
                ...
                $revsliders[__('No sliders found', 'wp-trust')] = 0;
            }
        return $revsliders;
        }
    }
