...
<div class="cms-counter-wraper cms-counter-layout-default <?php echo esc_attr($atts['template']);?>" id="<?php echo esc_attr($atts['html_id']);?>">
    ...
    <div class="cms-counter-body clearfix">
        <div class="cms-counter-content left">
            <span class="cmsline" style="background-color: <?php echo esc_attr($digit_color)?>"></span>
			<div style="color: <?php echo esc_attr($digit_color)?>" id="counter_<?php echo esc_attr($atts['html_id']);?>" class="cms-counter ft-lo dark2 <?php echo esc_attr(strtolower($atts['type']));?>" data-suffix="<?php echo esc_attr($atts['suffix']);?>" data-prefix="<?php echo esc_attr($atts['prefix']);?>" data-type="<?php echo esc_attr(strtolower($atts['type']));?>" data-digit="<?php echo esc_attr($atts['digit']);?>">
			</div>
            <?php if($atts['c_title']):?>
                <div class="cms-counter-title ft-r dark2" style="color: <?php echo esc_attr($title_color)?>">
                    <?php echo apply_filters('the_title',$atts['c_title']);?>
                </div>
            <?php endif;?>
        </div>
        <div class="cms-counter-icon right">
            <?php if( $icon_custom ): ?>
