...
function wc_products_rss_feed() {
	...
	if ( is_post_type_archive( 'product' ) || is_singular( 'product' ) ) {
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . esc_html__( 'New products', 'wp-trust' ) . '" href="' . esc_url( $feed ) . '" />';
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . sprintf(esc_html__( 'New products added to %s', 'wp-trust' ), urlencode($term->name)) . '" href="' . esc_url( $feed ) . '" />';
		...
		echo '<link rel="alternate" type="application/rss+xml"  title="' . sprintf(esc_html__( 'New products tagged %s', 'wp-trust' ), urlencode($term->name)) . '" href="' . esc_url( $feed ) . '" />';
...
if ( ! function_exists( 'woocommerce_demo_store' ) ) {
	...
	function woocommerce_demo_store() {
		...
		if ( empty( $notice ) )
			$notice = esc_html__( 'This is a demo store for testing purposes &mdash; no orders shall be fulfilled.', 'wp-trust' );
...
if ( ! function_exists( 'woocommerce_page_title' ) ) {
	...
	function woocommerce_page_title( $echo = true ) {
		...
		if ( is_search() ) {
			$page_title = sprintf( esc_html__( 'Search Results: &ldquo;%s&rdquo;', 'wp-trust' ), get_search_query() );

			if ( get_query_var( 'paged' ) )
				$page_title .= sprintf( esc_html__( '&nbsp;&ndash; Page %s', 'wp-trust' ), get_query_var( 'paged' ) );
...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
	...
	function woocommerce_catalog_ordering() {
		...
		$catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
			'menu_order' => esc_html__( 'Default sorting', 'wp-trust' ),
			'popularity' => esc_html__( 'Sort by popularity', 'wp-trust' ),
			'rating'     => esc_html__( 'Sort by average rating', 'wp-trust' ),
			'date'       => esc_html__( 'Sort by newness', 'wp-trust' ),
			'price'      => esc_html__( 'Sort by price: low to high', 'wp-trust' ),
			'price-desc' => esc_html__( 'Sort by price: high to low', 'wp-trust' )
		) );
	...
	function woocommerce_default_product_tabs( $tabs = array() ) {
		...
		if ( $post->post_content ) {
			$tabs['description'] = array(
				'title'    => esc_html__( 'Description', 'wp-trust' ),
				'priority' => 10,
		...
		if ( $product && ( $product->has_attributes() || ( $product->enable_dimensions_display() && ( $product->has_dimensions() || $product->has_weight() ) ) ) ) {
			$tabs['additional_information'] = array(
				'title'    => esc_html__( 'Additional Information', 'wp-trust' ),
				'priority' => 20,
		...
		if ( comments_open() ) {
			$tabs['reviews'] = array(
				'title'    => sprintf( esc_html__( 'Reviews (%d)', 'wp-trust' ), $product->get_review_count() ),
				'priority' => 30,
...
if ( ! function_exists( 'woocommerce_related_products' ) ) {
	...
	function woocommerce_related_products( $args = array(), $columns = false, $orderby = false ) {
		if ( ! is_array( $args ) ) {
			_deprecated_argument( __FUNCTION__, '2.1', esc_html__( 'Use $args argument as an array instead. Deprecated argument will be removed in WC 2.2.', 'wp-trust' ) );
...
if ( ! function_exists( 'woocommerce_button_proceed_to_checkout' ) ) {
	...
	function woocommerce_button_proceed_to_checkout() {
		...
		?>
		<a href="<?php echo esc_url($checkout_url); ?>" class="checkout-button btn wc-forward"><?php esc_html_e( 'Proceed to Checkout', 'wp-trust' ); ?></a>
		<?php
	}
}
...
if ( ! function_exists( 'woocommerce_breadcrumb' ) ) {
	...
	function woocommerce_breadcrumb( $args = array() ) {
		$args = wp_parse_args( $args, apply_filters( 'woocommerce_breadcrumb_defaults', array(
			...
			'before'      => '',
			'after'       => '',
			'home'        => _x( 'Home', 'breadcrumb', 'wp-trust' )
		) ) );
...
if ( ! function_exists( 'woocommerce_checkout_payment' ) ) {
	...
	function woocommerce_checkout_payment() {
		...
		wc_get_template( 'checkout/payment.php', array(
			...
			'available_gateways' => WC()->payment_gateways()->get_available_payment_gateways(),
			'order_button_text'  => apply_filters( 'woocommerce_order_button_text', esc_html__( 'Place order', 'wp-trust' ) )
		) );
	}
}
...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		if ( $args['required'] ) {
			...
			$required = ' <abbr class="required" title="' . esc_html__( 'required', 'wp-trust'  ) . '">*</abbr>';
		} else {
			$required = '';
		}
			...
			case 'country' :
				...
				if ( sizeof( $countries ) == 1 ) {
					...
					$field = '<p class="form-row ' . esc_attr( implode( ' ', $args['class'] ) ) .'" id="' . esc_attr( $args['id'] ) . '_field">'
							...
							. '<option value="">'.esc_html__( 'Select a country&hellip;', 'wp-trust' ) .'</option>';
					...
					$field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . esc_html__( 'Update country', 'wp-trust' ) . '" /></noscript>';
			...
			case 'state' :
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) .'" ' . implode( ' ', $custom_attributes ) . ' placeholder="' . esc_attr( $args['placeholder'] ) . '">
						<option value="">'.esc_html__( 'Select a state&hellip;', 'wp-trust' ) .'</option>';
					...
					foreach ( $args['options'] as $option_key => $option_text ) {
						if ( "" === $option_key ) {
							...
							if ( empty( $args['placeholder'] ) ) {
								$args['placeholder'] = $option_text ? $option_text : esc_html__( 'Choose an option', 'wp-trust' );
							}
